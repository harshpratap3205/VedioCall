...
//     initializeMedia();
//   }, [getUserMedia, localStream, isUserNameSet]);

//   // Set up event listeners - FIXED VERSION
//   useEffect(() => {
//     if (!socket || !isConnected || !localStream || !isUserNameSet || eventListenersSetRef.current) {
//       return;
//     }
...
//     const handleUserJoined = ({ userId, userName: joinedUserName }) => {
//       console.log(`User joined: ${joinedUserName} (${userId})`);
//       showToast(`${joinedUserName} joined the call`);
...
//           const handleIceCandidate = (candidate) => {
//             safeEmit('ice-candidate', { candidate, targetUserId: userId });
//           };
...
//               if (offer) {
//                 console.log("Sending offer to:", userId);
//                 safeEmit('offer', { offer, targetUserId: userId });
//               }
//             })
//             .catch(err => {
//               console.error("Error creating peer connection:", err);
...
//       try {
//         const handleIceCandidate = (candidate) => {
//           safeEmit('ice-candidate', { candidate, targetUserId: fromUserId });
//         };
...
//         if (answer) {
//           console.log("Sending answer to:", fromUserId);
//           safeEmit('answer', { answer, targetUserId: fromUserId });
          
//           setParticipants(prev => {
//             const exists = prev.some(p => p.id === fromUserId);
//             if (!exists) {
...
//     safeEmit('join-room', { roomId, userName });
...
//     };
//   }, [socket, isConnected, localStream, isUserNameSet, roomId, userName, safeEmit, on, off, showToast, createPeerConnection, createOffer, handleOffer, handleAnswer, addIceCandidate, closePeerConnection, hasPeerConnection]);
...
//     toggleAudio();
//     safeEmit('audio-toggle', { isEnabled: !isAudioEnabled });
//   }, [toggleAudio, isAudioEnabled, safeEmit]);

//   const handleToggleVideo = useCallback(() => {
//     toggleVideo();
//     safeEmit('video-toggle', { isEnabled: !isVideoEnabled });
//   }, [toggleVideo, isVideoEnabled, safeEmit]);
...
//       showToast('Failed to share screen', 'error');
//     }
//   }, [isScreenSharing, stopScreenShare, startScreenShare, showToast]);
...
//     closeAllPeerConnections();
//     navigate('/');
//   }, [safeEmit, stopMedia, closeAllPeerConnections, navigate]);
...
//     setDiagnosticData(data);
//   }, [isConnected, socket, socketError, localStream, mediaError, roomId, participants, userName, connectionStatus, remoteStreams]);
...
//       console.log(`Setting up video for ${userId}`);
//       videoRef.current.srcObject = stream;
...
//         // Ensure remote videos are not muted
//         if (userId !== 'local' && videoRef.current) {
//           videoRef.current.muted = false;
//           videoRef.current.volume = 1.0;
//         }
//       };
...
//               mirrored={userId === 'local'}
//             />
//             {(isLoading || !stream.getVideoTracks().length) && (
//               <VideoPlaceholder>
//                 {name?.charAt(0)?.toUpperCase() || '?'}
//               </VideoPlaceholder>
//             )}
...
//       }
//     };
//   }, [stopMedia, closeAllPeerConnections, socket, isConnected, safeEmit]);
...
//             autoFocus
//           />
//           <UsernameButton onClick={handleUsernameSubmit}>
//             Join Call
//           </UsernameButton>
//         </UsernameForm>
//       </UsernameModal>
...
//           <ErrorHeading>Connecting...</ErrorHeading>
//           <ErrorMessage>
//             {!localStream ? 'Accessing camera and microphone...' : 'Joining room...'}
//           </ErrorMessage>
//         </ErrorOverlay>
//       </VideoContainer>
//     );
...
//         </div>
//         <div style={{ marginBottom: '0.5rem' }}>
//           <strong>Participants:</strong> {participants.length + 1}
//         </div>
...
//       {showDiagnostics && (
//         <DiagnosticOverlay>
//           <DiagnosticClose onClick={() => setShowDiagnostics(false)}>
//             Close
//           </DiagnosticClose>
//           {renderDiagnostics()}
//         </DiagnosticOverlay>
